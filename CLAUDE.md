...
- 当前状态：**v3.0 全部完成,项目功能完整**。六章主交互 + 分屏对比实验室 + 挑战任务/成就系统(localStorage 持久化)+ 移动端滑动/键盘导航 + 可访问性收尾均已实现。
...
1. **alpha** ✅：第一章 + 第二章（聚焦模式、慢动作、分步推导向导）。
2. **beta** ✅：第三章频谱双向联动 + 随机化相位 + Web Audio 视听同步。
3. **v1.0** ✅：第四章周期拉伸、参数动态显示、显微镜工具、∑→∫ 飞跃。
4. **v2.0** ✅：终章神笔傅里叶（分步绘制 + 二维频谱 + 知识桥接）。
5. **v3.0** ✅：序幕乐器音色对比、移动端左右滑动手势、键盘切章、可访问性收尾(prefers-reduced-motion / 焦点态 / 跳转链接)、分屏对比实验室、挑战任务/成就系统(localStorage 持久化)。
...
└── src/
    ...
    │   ├── ComparisonLab.jsx # 分屏对比实验室(左右独立波形/谐波数实时对比)
    │   └── AchievementPanel.jsx # 成就面板(徽章解锁进度)
    ├── achievements/       # 成就系统(Context + localStorage 持久化)
    │   └── AchievementContext.jsx  # 徽章定义、unlock 上报、toast
    └── chapters/           # 各章节模块，互相解耦
