...
- 当前状态：**六章主交互全部完成**（序幕 + 第一至四章 + 终章），含 Web Audio 视听同步、∑→∫ 周期拉伸、神笔傅里叶 2D DFT 绘图、移动端滑动/键盘导航与可访问性收尾。v3.0 剩余打磨项：分屏对比实验室、挑战任务系统。
...
5. **v3.0** ✅(部分)：序幕乐器音色对比、移动端左右滑动手势、键盘切章、可访问性收尾(prefers-reduced-motion / 焦点态 / 跳转链接)。
   - 待办：分屏对比实验室、挑战任务/成就系统。
